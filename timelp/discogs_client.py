...
class Release(APIBase):
	...
	def thumb(self):
		if not self._thumb:
			self._thumb = self.data.get('thumb')
		return self._thumb
...
class User(APIBase):
	...
	def collection(self, sort=None, order=None, page=None, per_page=None):
		self._path = '/users/%s/collection/folders/0/releases' % self._username
		self._clear_cache()
		self._params = {'sort': sort, 'sort_order': order, 'page': page, 'per_page': per_page}
		collection = []
		for r in self.data.get('releases'):
